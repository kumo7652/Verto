...
public class NettyTransportClient {
     ...
     * <h3>发送 RPC 请求</h3>
     *
     * @param bodyBytes   已序列化的请求体
     * @param serviceNode 目标服务节点
     * @param serializer  序列化器别名
     * @return 异步响应 Future（持有序列化后的响应体 byte[]）
     */
    public CompletableFuture<byte[]> send(byte[] bodyBytes, ServiceNode serviceNode, String serializer) {
        long requestId = RequestIdGenerator.nextId();
        String host = serviceNode.getServiceHost();
        int port = serviceNode.getServicePort();
        String endpoint = host + ":" + port;
        CompletableFuture<byte[]> future = dispatcher.register(requestId, config.getResponseTimeoutMs(), endpoint);
        ...
        connection.thenAccept(channel -> {
            ...
            try {
                byte code = SerializerFactory.getInstance().getCodeByName(serializer);
                VertoPacket.Header header = VertoPacket.requestHeader(requestId, code);
                VertoPacket packet = new VertoPacket(header, bodyBytes);
