...
public class SerializerFactory {
    ...
    private void loadFromExtension() {
        ...
        for (Map.Entry<String, Class<?>> entry : loadedClasses.entrySet()) {
            ...
            int code = ExtensionLoader.getCode(Serializer.class, name);

            if (codeToName.containsKey((byte) code)) {
                log.warn("序列化器编号冲突: {} 已被 {} 占用，跳过 {}",
                        code, codeToName.get((byte) code), name);
                continue;
            }

            codeToName.put((byte) code, name);
            nameToCode.put(name, (byte) code);

            Serializer instance = ExtensionLoader.getInstance(Serializer.class, name);
            codeToInstance.put((byte) code, instance);
